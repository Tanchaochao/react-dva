...
export default function deleteIn(state, field) {
  ...
  for (; i < len - 1; i++) {
    if (!isArrayPath(path[i]) && Array.isArray(parentState)) {
      throw new Error(`Must access array elements with a number, not "${String(path[i])}".`);
    }
  ...
  if (!isArrayPath(last) && Array.isArray(parentState)) {
    throw new Error(`Cannot delete non-numerical index from an array. Given: "${String(last)}`);
  }
